	...
	while (inputcnt && outputcnt) {
		...
		case 2:	/* Waiting for )        */
			if (c == ')' || c == '}') {
				...
				// Hacked by FriendlyARM, for filesize_aligned macro
				if (strcmp(envname, "filesize_aligned") == 0) {
					unsigned filesize_aligned = 0;
					#define nand_page_size 2048
					char *filesize_str = getenv("filesize");
					static char filesize_aligned_str[10];
					if (filesize_str) {
						char *endp;
						filesize_aligned = simple_strtoul(filesize_str, &endp, 16);
					}
					if (filesize_aligned % nand_page_size != 0) {
						filesize_aligned /= nand_page_size;
						filesize_aligned++;
						filesize_aligned *= nand_page_size;
					}
					sprintf(filesize_aligned_str, "%x", filesize_aligned);
					envval = filesize_aligned_str;
				}
				// Hacked end
